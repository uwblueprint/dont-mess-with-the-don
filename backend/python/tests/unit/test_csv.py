...
def test_transform():
    ...
    assert (
        result == "name,age,pets,num_pets\r\nPerson1,20,"
        "\"[{'name': 'Beans', 'type': 'Cat'}, {'name': 'Spot', 'type': 'Dog'}]\",2\r\n"
    )
...
def test_flatten_lists():
    ...
    assert (
        result == "name,age,pets.0,pets.1\r\nPerson1,20,"
        "\"{'name': 'Beans', 'type': 'Cat'}\",\"{'name': 'Spot', 'type': 'Dog'}\"\r\n"
    )
...
def test_flatten_both():
    ...
    assert (
        result == "name,age,pets.0.name,pets.0.type,pets.1.name,pets.1.type\r\n"
